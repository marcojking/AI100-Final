...
X_test_scaled = scaler.transform(X_test)           # <-- correct: transform only

device = torch.device('cpu')
X_train_t = torch.FloatTensor(X_train_scaled).to(device)
y_train_t  = torch.LongTensor(y_train).to(device)  # <-- correct: LongTensor
...
model = GenreClassifier(input_size=13, num_classes=10).to(device)
