...
class GenreClassifierNoReLU(nn.Module):   # BUG: ReLU removed
    def __init__(self, input_size, num_classes):
        super().__init__()
        self.network = nn.Sequential(
            nn.Linear(input_size, 256), nn.BatchNorm1d(256), nn.Dropout(0.3),  # no ReLU
            nn.Linear(256, 128),        nn.BatchNorm1d(128), nn.Dropout(0.3),  # no ReLU
            nn.Linear(128, 64),         nn.BatchNorm1d(64),  nn.Dropout(0.2),  # no ReLU
            nn.Linear(64, num_classes)
        )
    def forward(self, x): return self.network(x)

model = GenreClassifierNoReLU(13, 10)
criterion = nn.CrossEntropyLoss()
optimizer = torch.optim.Adam(model.parameters(), lr=0.001, weight_decay=1e-4)

print("=== BUG 07: No ReLU activations (model is effectively linear) ===")
EPOCHS = 5
for epoch in range(EPOCHS):
    ...
    print(f"Epoch {epoch+1}/{EPOCHS}  loss={total_loss/len(train_loader):.4f}  test_acc={acc:.4f}")
print("\nExpected: lower accuracy than base model (no non-linearity to learn complex patterns)")
